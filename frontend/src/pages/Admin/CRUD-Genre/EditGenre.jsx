...
export default function CreateGenre() {
  ...
  useEffect(() => {
    AdminService.getDetailsGenres(genreId).then((response) => {
      setGenre(response.data.Genre.Genre_Cerita);
    });
  }, []);
  const updateArticles = async (e) => {
    e.preventDefault();
    try {
      const response = await AdminService.updateGenreCerita(genreId, genre);
      if (response) {
        alert("UPDATE succes");
